...
export class BpmnEngineClient {
  ...
  /**
   * Read the short human-readable label for a process instance, or null
   * when none has been set yet. Hosts use the `updatedAt` field as a
   * dirty-check baseline before regenerating.
   *
   * Policy (when to regenerate, how to phrase, language, length-in-words)
   * lives in the host; this is a thin DB read.
   */
  async getInstanceSynopsis(
    instanceId: string
  ): Promise<import('./types').InstanceSynopsis | null> {
    if (this.config.mode === 'rest') {
      const res = await fetch(
        `${this.config.baseUrl}/v1/instances/${encodeURIComponent(instanceId)}/synopsis`
      );
      if (res.status === 404) return null;
      if (!res.ok) throw new Error(`Get instance synopsis failed: ${res.status}`);
    ...
    const { getInstanceSynopsis } = await import('../instance/service');
    return getInstanceSynopsis(this.config.db, instanceId);
  }

  /**
   * Write the short human-readable label for a process instance. `source`
   * is `auto` when generated by the host's hook, `manual` when pinned by
   * a user — the host's auto-update path must skip when current source
   * is `manual`. Throws if `text` is empty or exceeds the platform-level
   * upper bound (`INSTANCE_SYNOPSIS_MAX_CHARS`, 200 chars). Hosts should
   * enforce a tighter display ceiling in their prompt; the platform bound
   * is the absolute backstop.
   *
   * Returns `true` if the row existed and was updated, `false` if no row
   * matched `instanceId`.
   */
  async setInstanceSynopsis(
    instanceId: string,
    text: string,
    ...
    if (this.config.mode === 'rest') {
      const res = await fetch(
        `${this.config.baseUrl}/v1/instances/${encodeURIComponent(instanceId)}/synopsis`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ text, source: options.source }),
        }
      );
      if (res.status === 404) return false;
      if (!res.ok) {
        const err = await res.json().catch(() => ({ error: res.statusText }));
        throw new Error((err as { error?: string }).error ?? `Set synopsis failed: ${res.status}`);
      }
      return true;
    }
    const { setInstanceSynopsis } = await import('../instance/service');
    return setInstanceSynopsis(this.config.db, instanceId, text, options);
  }

  /**
   * Ordered list of `TASK_COMPLETED` history entries for an instance —
   * the "what has happened on this case so far" view. Generalized
   * previous-task helper used by synopsis generation, audit views, and
   * the eventual "explain this case" surface.
   *
   * `limit` caps the most-recent N entries (default: all). `includeData`
   * controls whether activity result payloads are returned (default: no).
   */
  async completedActivities(
    instanceId: string,
    options?: { limit?: number; includeData?: boolean }
  ): Promise<import('./types').CompletedActivity[]> {
    if (this.config.mode === 'rest') {
      ...
      const res = await fetch(
        `${this.config.baseUrl}/v1/instances/${encodeURIComponent(instanceId)}/completed-activities${qs ? `?${qs}` : ''}`
      );
      if (res.status === 404) return [];
      if (!res.ok) throw new Error(`Get completed activities failed: ${res.status}`);
      ...
      return data.map(d => ({ ...d, completedAt: new Date(d.completedAt) }));
    }
    const { completedActivities } = await import('../instance/service');
    return completedActivities(this.config.db, instanceId, options);
  }
