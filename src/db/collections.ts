...
export type InstanceSynopsisSource = 'auto' | 'manual';
...
export type ProcessInstanceDoc = {
  ...
  startedByDetails?: UserDetails;
  /**
   * Short human-readable label for this instance, displayed in worklists and
   * process-instance lists. Generated by the host (e.g. via an LLM call on
   * task completion) and written through `setInstanceSynopsis`. Absent on
   * legacy instances; consumers fall back to definition name + createdAt.
   */
  instanceSynopsis?: string;
  /** Set on every synopsis write. Hosts use it as the dirty-check baseline. */
  instanceSynopsisUpdatedAt?: Date;
  /**
   * `auto` — written by the host's hook. `manual` — pinned by a user. The
   * auto-update path must skip when source is `manual`.
   */
